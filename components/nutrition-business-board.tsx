...
export function NutritionBusinessBoard({
  ...
  return (
    <section className="panel-strong rounded-[2rem] p-7 lg:p-8">
      <p className="eyebrow">Base del negocio</p>
      <div className="mt-3 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="headline text-3xl font-semibold text-slate-950">La lógica clínica, visible como hoja operativa</h2>
        ...
        <div className="flex flex-wrap gap-2">
          <Link href={`/patients/${patientId}?tab=measurements`} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:border-slate-300">
            Peso / TMB
          </Link>
          <Link href={`/patients/${patientId}?tab=goals`} className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:border-slate-300">
            Requerimientos
          </Link>
          <Link href={latestPlanId ? `/plans/${latestPlanId}` : `/patients/${patientId}?tab=plans`} className="rounded-full bg-[#0f5c4d] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0a4a3d]">
            Menú cuantificado
          </Link>
        </div>
      </div>
      ...
      <article className="mt-6 rounded-[1.6rem] border border-slate-200 bg-white/70 p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <Link href={latestPlanId ? `/plans/${latestPlanId}` : `/patients/${patientId}?tab=plans`} className="rounded-full border border-[#0f5c4d]/18 bg-white px-4 py-2 text-sm font-semibold text-[#0f5c4d] hover:bg-[#f1f7f4]">
            Abrir hoja completa
          </Link>
        </div>
        <div className="mt-4 overflow-x-auto">
